...
class Trainer:
    ...
    It has two modes: solo and multi, which is set in constructor.
    Methods:
        train: train a model
    ...
    def train(self, config: dict = {}) -> None:
        ...
        for mode, config in configs.items():
            if config is not None:
                ...
                self.models[mode]["model"].fit(
                    train_X, train_y, **config.get("fit_config", {})
                )
...
if __name__ == "__main__":
    ...
    trainer.train(
        config={"solo": {"model_type": "cat", "fit_config": {"verbose": 200}}}
    )
